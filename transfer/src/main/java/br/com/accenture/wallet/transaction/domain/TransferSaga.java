...
import static br.com.accenture.wallet.transaction.domain.command.WithdrawnCommand.TRANSFER;
...
public class TransferSaga {
    ...
    private CommandCallback<WithdrawnCommand, Object> withdrawnHandler = (message, result) -> {
        if (result.isExceptional()) {
            log.error(result.exceptionResult().getMessage(), result.exceptionResult());
            commandGateway.send(new TransferCancelCommand(transferId));
        }
    };
    ...
    public void onTransferRequested(TransferRequestedEvent event) {
        ...
        final WithdrawnCommand cmd = new WithdrawnCommand(TRANSFER)
    ...
    public void onCacelTransfer(TransferCancelledEvent event) {
        log.debug("Cancelling transfer flow.");
        SagaLifecycle.end();
    }
